...
<script type="text/javascript">
	...
	function doAction(type) {
		...
		case 'A':
			...
			window.open("${pageContext.request.contextPath }/AdminAddDrinkForm.do",
					'window', 'width=900, height=500, menubar=no, status=no, toolbar=no , left=500, top=50');
	    	  }
			...
			var boardIdxArray = [];
	    	  ...
	    	  if (boardIdxArray == ""){
	    		  ...
	    		  $.ajax({
	    	            ...
	    	            type : "POST",
	    	            url : "${pageContext.request.contextPath }/AdminUpdateDrinkForm.do",
	    	            data : params,
	    	            error : function(){
	    	                alert('통신실패!!');
	    	            },
	    	            success : function(data){
	    	                alert("수정할 음료 이름 : " + data) ;
	    	            }
	    		  ...
	    		  */
	    		  alert(seqArr)
	  			window.open("${pageContext.request.contextPath }/AdminUpdateDrinkForm.do?dk_name="+seqArr,
	  					'window', 'width=900, height=500, menubar=no, status=no, toolbar=no'); 
	    	  }
						break;
	    			...
	    			if (answer) {
	  	    		  ...
	  	    		  $.ajax({
	  	    	            ...
	  	    	            success : function(data){
	  	    	              ...
	  	    	              alert('총 ' + data.trim() + '개 음료 삭제되었습니다')
		...
		<div align="center">
			<div class="page-head-content" align="center">
				...
				<table border="1" class="list">
					<tr>
						<th width="3%">전체 선택 <label class="checkbox-inline">
								<input type="checkbox" id="allCheckBox" name="cchk0" value="GGGGG">
						</label>
						</th>
						<th width="9%">메뉴 이름</th>
						<th width="4%">아이스/핫</th>
					...
					<c:forEach items="${ allDrinkList }" var="drink">
						<tr>
							<td><label class="checkbox-inline"> <input
									type="checkbox" name="cchk" onclick="cchkClicked()" value="${ drink.dk_name }">
							</label></td>
