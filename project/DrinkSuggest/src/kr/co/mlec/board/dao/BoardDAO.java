...
public class BoardDAO {
	...
	public List<BoardVO> selectAllboard(PagingVO paging) {
		...
		try {
			...
			sql.append(
					"select * from (select  rownum rnum, b.*from(select comNo, id, comTitle, comContent, viewCnt, regDate, comLikeCnt  ");
	...
	public BoardVO selectBoardByNo(int boardNo) {
		...
		try {
			...
			if (rs.next()) {
				...
				board = new BoardVO(comNo, id, comTitle, comContent, comLikeCnt, comRegDate, viewCnt); // 상세조회된 애들을
																										// board로 묶는다.
			}
			// rs.next();
		} catch (Exception e) {
	...
	/*
	 * 
	 * 현재 그룹넘버 가장 마지막 숫자 추출해서 1 더하기 새글 등록에 쓸 것...
	 * 
	 */
	...
	/*
	 * 
	 * 현재 reNo 가장 마지막 숫자 추출해서 1 더하기 새글 등록에 쓸 것...
	 * 
	 */
